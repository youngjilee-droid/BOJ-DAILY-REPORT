...
# ─── 페이지 설정 ───────────────────────────────────────────────
st.set_page_config(page_title="BOJ | 코멘트 생성", layout="wide", page_icon="📝")
...
with st.spinner("데이터 로딩 중..."):
    try:
        ...
        st.success(f"✅ 데이터 로드 완료 | 총 {len(df_raw):,}행 | 날짜 범위: {earliest_date} ~ {latest_date}")
    except Exception as e:
        st.error(f"파일 로드 실패: {e}")
        st.stop()

# 날짜 유효성 검사 — 파일 로드 후 바로 체크
if target_date > latest_date:
    st.error(
        f"⚠️ **선택한 날짜({target_date})의 데이터가 파일에 없습니다.**\n\n"
        f"업로드된 파일의 최신 데이터: **{latest_date}**\n\n"
        f"**해결 방법 ①** 사이드바에서 날짜를 `{latest_date}` 이하로 변경\n\n"
        f"**해결 방법 ②** {target_date} 데이터가 포함된 최신 리포트 파일을 업로드"
    )
    st.stop()
elif target_date < earliest_date:
    st.warning(f"⚠️ 선택한 날짜({target_date})가 파일 데이터 시작일({earliest_date})보다 이전입니다.")
...
if st.button("🚀 코멘트 생성", type="primary", use_container_width=True):
    ...
    for idx, media_key in enumerate(selected_media):
        ...
        # 해당 날짜 데이터 없으면 명확히 알림
        if today_data['spend'] == 0 and today_data['purchase'] == 0 and today_data['revenue'] == 0:
            media_dates = sorted(df_media['날짜'].dt.date.dropna().unique())
            latest_m = max(media_dates) if media_dates else "없음"
            st.warning(f"⚠️ **{media_key}**: {target_date} 데이터 없음 (해당 매체 최신 날짜: {latest_m})")
            st.divider()
            continue
